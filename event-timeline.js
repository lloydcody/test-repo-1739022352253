...
function loadGoogleSheetsAPI() {
  ...
  class AnimationTimeline {
    ...
    this.events.forEach(event => {
        ...
        const eventTime = parseInt(Math.floor(event.time/1000));
        sequencerImportedCues.push({
            key: event.time + '_' + event.target,
            time: eventTime,
            callback: () => {
  ...
  function updateDom(targetSelector, classNameToRemove, classNameToAdd) {
    ...
    if (classNameToRemove && classNameToRemove.endsWith('*')) {
        ...
        console.log('Removing from', targetEl, 'class', classNameToRemove);
    } else if (classNameToRemove) {
        console.log('Removing from', targetEl, 'class', classNameToRemove);
